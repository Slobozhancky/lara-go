...
```
## Звязки між моделями
- Тут більше загальна інформація, а саме що звязки є `one-to-one, one-to-many, many-to-many`
- Забезпечують ці звязки безпосередньо налаштування в самих моделях нах сутностей
- Щоб отримати екземпляр звязків, слід викликати звязаний метод моделі:
> !!! Якщо зіштовнувся з проблемою виклику екземплярів, або взагалі сутностей звязків, дивись, щоб вірно були прописані назви методів, або таблиць. бо ОДНИНА і МНОЖИНА за конвенцією багато чого рішає :)

### Виклик як метод
- Це дозволяє користуватись методами Query Builder
```php
    $author_books = Author::query()->find(2);
    $book_author = Book::query()->find(201);
    dump($author_books->books());
    dump($book_author->author());
```
- Ось такий приклад виклику, як метода на знайденій моделі, поверне нам ![ієрархію звязків в цих моделях](https://i.imgur.com/rKXl5xZ.png)
- Можемо звернути увагу, що нам повернеться [звязок з моделю Builder](https://i.imgur.com/iayqu4X.png) - та це говорить нам про те, що ми можемо для Query Builder, викликати методи, для роботи з базою
- Але тут слід звенути увагу, що підказаок IDE не буде давати, тому не пугаємся
- Результатом, ми побачимо відсортований масив даних
```php
$author_books->books()->orderBy('id', 'asc')->get()->toArray()
```

### Виклик як властивість
- Тут ми можемо побачити, що наші моделі, будуть повернуті як Collection
- Та це дозволяє, нам використовувати [методи колекцій](https://laravel.com/docs/10.x/collections)
```php
$author_books->books->where('id', '!=', 202)
```
- Це дозволяє зручно підготувати інформацію для виводу 
...
2. **user -> posts**
    - Один користувач може мати багато публікацій.

3. **order -> items**
    - Одне замовлення може містити багато товарних позицій.
...
6. **company -> employees**
    - Одна компанія має багато співробітників.

### Порядок налаштування звязків

Щоб організувати зв'язок "один до багатьох" в Laravel, дотримуйтесь таких кроків:

1. **Створіть моделі**:
    - Визначте дві моделі, між якими буде встановлений зв'язок "один до багатьох". Наприклад, якщо у вас є моделі `Category` і `Product`, одна категорія може мати багато товарів.

2. **Створіть міграції**:
    - Створіть таблиці для кожної моделі. Таблиця, яка містить зв'язок, повинна мати зовнішній ключ, що посилається на первинний ключ іншої таблиці.
    - Зовнішній ключ буде зберігатися в таблиці, яка містить багато записів (наприклад, в таблиці `products` для зв'язку з `categories`).

3. **Додайте методи відносин в моделі**:
    - У моделі, яка має зовнішній ключ (наприклад, `Product`), додайте метод `belongsTo` для встановлення зв'язку з іншою моделлю (`Category`).
    - У моделі, яка має один запис, додайте метод `hasMany` для встановлення зв'язку з іншою моделлю, що містить багато записів.

4. **Налаштуйте міграції**:
    - Вкажіть зовнішній ключ у відповідній міграції. Він повинен посилатися на первинний ключ іншої таблиці. Налаштуйте поведінку при видаленні або оновленні запису, використовуючи `onDelete` або `onUpdate`.

5. **Використовуйте відносини в коді**:
    - Після налаштування зв'язку, ви можете використовувати його для отримання всіх пов'язаних записів. Наприклад, ви можете отримати всі товари, що належать до певної категорії, або отримати категорію, до якої належить певний товар.

Ці кроки дозволяють налаштувати зв'язок "один до багатьох" між двома моделями у Laravel, забезпечуючи правильну структуру бази даних і доступ до пов'язаних даних.

## Проблема n+1

Проблема **N+1 запитів** виникає, коли ви виконуєте занадто багато запитів до бази даних під час отримання пов'язаних даних. У Laravel ця проблема часто виникає при використанні Eloquent ORM для завантаження пов'язаних моделей.

### Приклад проблеми N+1

Припустимо, у вас є таблиця `posts` (пости) та таблиця `comments` (коментарі), де кожен пост має багато коментарів. Якщо ви хочете отримати всі пости разом з їхніми коментарями, то ви можете зробити це наступним чином:
...
foreach ($posts as $post) {
    foreach ($post->comments as $comment) {
        // Виконується для кожного коментаря
    }
}
```

Це призведе до такої ситуації:

1. Один запит для отримання всіх постів: `SELECT * FROM posts`.
2. І для кожного поста виконується окремий запит для отримання коментарів: `SELECT * FROM comments WHERE post_id = ?`.

Якщо у вас є 100 постів, це створить 1 основний запит для отримання всіх постів і 100 додаткових запитів для кожного поста, щоб отримати коментарі. Це і є проблема N+1.

### Як вирішити проблему N+1 в Laravel

Laravel надає простий спосіб вирішення цієї проблеми за допомогою **жадібного завантаження** (eager loading). Жадібне завантаження дозволяє завантажувати всі необхідні пов'язані дані за допомогою одного додаткового запиту.

### Приклад вирішення:

Замість того щоб робити по одному запиту для кожного поста, ви можете використати метод `with()`:
...
Це виконає лише два запити:

1. `SELECT * FROM posts` - для отримання всіх постів.
2. `SELECT * FROM comments WHERE post_id IN (1, 2, 3, ..., 100)` - для отримання всіх коментарів, пов'язаних з цими постами.

### Основні моменти:

- **N+1 запити:** Це коли для кожного рядка у наборі даних виконується додатковий запит для отримання пов'язаних даних.
- **Жадібне завантаження (Eager Loading):** Використовуйте метод `with()` в Eloquent, щоб уникнути N+1 запитів, завантажуючи всі необхідні дані за один раз.
- **Ліниве завантаження (Lazy Loading):** Це коли зв’язки завантажуються тільки тоді, коли вони потрібні. Це може призвести до проблеми N+1.

### Приклад n+1

- Вдалось відтворити складність повязаних між собою моделей, та нарешті допетрав в чому була моя бідося:
- Я коли хотів отримати всіх Авторів, та відразу отримати книги цих авторів, то трішки обкакавсь на моменті виводу інофрмації книг цих авторів. А саме я намагався застосувати метод books(), відразу ВСІМ авторам, а не кожному автору окремо
- І от коли почав метод, books() застосовувати кожному автору окремо, все почало працювати успішно, та я міг відобразити, ВСІ книги, по кожному автору окремо
- Та звісно зіштовхнувся з проблемою **N+1**, чого так дуже хотів :) ![Приклад проблеми](https://i.imgur.com/bWfHInR.png) - про що ця проблема? Тут на кожній новій ітерації циклу, відбувається запит в базу, щоб отримати список книг кожного, нового автора
```php
...
- Це процес описаний в [документації](https://laravel.com/docs/10.x/eloquent-relationships#eager-loading)
- Для цього можна використати метод with()
- [Приклад](https://i.imgur.com/LVDWTGy.png) як воно виглядає в загальній картині
- А тут видно, що тепер [буде виконуватись лише один запит](https://i.imgur.com/X3QYBuV.png), вмісто 10: 
```php
...
- Якщо метод `with()` допоміг нам вирішити проблему N + 1, то є метод withCount() - він дозволить підрахувати кількість елементів у зв`язаній таблиці. Тобто, це вирішить проблему, щоб відобразити кількість елементів, але не проблему N + 1
- А підрахунок елементів, відбувається таким чином, що в обєкт моделі, буде додана властивість з назвою звязаної таблиці з приставкою `count`
- Приклад запиту: 
```php
        $authors = Author::withCount('books')->get();
        dump($authors);
```
- Те як буде виглядати якщо [задампити дані](https://i.imgur.com/v0nlizg.png), тобто бачимо що зявилась властивість `books_count`
```php
 $authors = Author::with('books')->get();
...
### Що таке зв'язок "багато до багатьох" (many-to-many)?

Зв'язок "багато до багатьох" означає, що одна модель може бути пов'язана з багатьма іншими моделями, і навпаки. Наприклад, уявімо собі систему, де в нас є таблиці `products` (товари) і `categories` (категорії). Один товар може належати до багатьох категорій, і одна категорія може включати багато товарів.

### Як реалізувати зв'язок "багато до багатьох" у Laravel?

### 1. **Створення таблиць**

Для реалізації зв'язку "багато до багатьох" потрібна проміжна таблиця, яка містить зовнішні ключі з двох таблиць. Ця таблиця часто називається відповідно до назви двох таблиць, які вона пов'язує. Наприклад, для `products` та `categories` ми можемо створити таблицю `category_product`.
...
#### Проміжна таблиця `category_product`
...
### 2. **Визначення зв'язку в моделях**

У моделях `Product` та `Category` ми повинні визначити методи, які будуть встановлювати зв'язок "багато до багатьох".
...
### 3. **Використання зв'язку "багато до багатьох"**

Тепер, коли зв'язок встановлено, ми можемо використовувати його для отримання або додавання даних.

#### Отримання категорій для товару

```php
$product = Product::find(1); // Знайти товар з ID 1
$categories = $product->categories; // Отримати всі категорії цього товару
```

#### Отримання товарів для категорії

```php
$category = Category::find(1); // Знайти категорію з ID 1
$products = $category->products; // Отримати всі товари в цій категорії
```

#### Додавання категорії до товару

```php
$product = Product::find(1);
$product->categories()->attach(2); // Додати категорію з ID 2 до цього товару
```

#### Видалення категорії з товару

```php
$product = Product::find(1);
$product->categories()->detach(2); // Видалити категорію з ID 2 з цього товару
```

### Пояснення

- **Проміжна таблиця:** Таблиця `category_product` служить для зберігання зв'язків між товарами та категоріями.
- **Методи `belongsToMany`:** Ці методи у моделях `Product` і `Category` визначають зв'язок між ними.
- **`attach` та `detach`:** Ці методи використовуються для додавання або видалення записів у проміжній таблиці, що відповідають за зв'язок між двома моделями.

### Приклади звязків Many to Many

### 1. **Students --> Courses**
- **Сценарій:** Один студент може бути записаний на багато курсів, і один курс може мати багато студентів.
- **Таблиці:**
    - **students**
    - **courses**
    - **course_student** (проміжна таблиця)
- **Моделі:**
    - `Student` має метод `courses()`
    - `Course` має метод `students()`

### 2. **Authors --> Books**
- **Сценарій:** Один автор може написати багато книг, і одна книга може бути написана багатьма авторами.
- **Таблиці:**
    - **authors**
    - **books**
    - **author_book** (проміжна таблиця)
- **Моделі:**
    - `Author` має метод `books()`
    - `Book` має метод `authors()`

### 3. **Users --> Roles**
- **Сценарій:** Один користувач може мати багато ролей, і одна роль може бути присвоєна багатьом користувачам.
- **Таблиці:**
    - **users**
    - **roles**
    - **role_user** (проміжна таблиця)
- **Моделі:**
    - `User` має метод `roles()`
    - `Role` має метод `users()`

### 4. **Movies --> Actors**
- **Сценарій:** Один фільм може мати багато акторів, і один актор може зніматися в багатьох фільмах.
- **Таблиці:**
    - **movies**
    - **actors**
    - **actor_movie** (проміжна таблиця)
- **Моделі:**
    - `Movie` має метод `actors()`
    - `Actor` має метод `movies()`

### 5. **Tags --> Posts**
- **Сценарій:** Один тег може бути присвоєний багатьом постам, і один пост може мати багато тегів.
- **Таблиці:**
    - **tags**
    - **posts**
    - **post_tag** (проміжна таблиця)
- **Моделі:**
    - `Tag` має метод `posts()`
    - `Post` має метод `tags()`

### 6. **Projects --> Developers**
- **Сценарій:** Один проект може мати багато розробників, і один розробник може працювати над багатьма проектами.
- **Таблиці:**
    - **projects**
    - **developers**
    - **developer_project** (проміжна таблиця)
- **Моделі:**
    - `Project` має метод `developers()`
    - `Developer` має метод `projects()`

### 7. **Books --> Genres**
- **Сценарій:** Одна книга може належати до кількох жанрів, і один жанр може містити багато книг.
- **Таблиці:**
    - **books**
    - **genres**
    - **book_genre** (проміжна таблиця)
- **Моделі:**
    - `Book` має метод `genres()`
    - `Genre` має метод `books()`

### 8. **Events --> Attendees**
- **Сценарій:** Один захід може мати багато учасників, і один учасник може відвідувати багато заходів.
- **Таблиці:**
    - **events**
    - **attendees**
    - **attendee_event** (проміжна таблиця)
- **Моделі:**
    - `Event` має метод `attendees()`
    - `Attendee` має метод `events()`


## Database: Seeding

**Database Seeding** у Laravel — це процес заповнення бази даних стартовими або тестовими даними. Сіди дозволяють вам створювати початкові дані для вашої програми, які можна використовувати для тестування або налаштування початкового стану бази даних. Це корисно для швидкого заповнення таблиць без необхідності вручну вводити дані.

**Eloquent Factories** — це інструмент для створення моделей та пов'язаних з ними даних. Фабрики допомагають швидко генерувати фейкові дані для тестування, за допомогою налаштованих шаблонів для створення моделей з випадковими даними. Це дозволяє легко тестувати різні сценарії без необхідності вручну створювати дані.

### Де почитати
- Про **Database Seeding** можна прочитати в [офіційній документації Laravel тут](https://laravel.com/docs/10.x/seeding).
- Про **Eloquent Factories** можна дізнатися більше в [офіційній документації Laravel тут](https://laravel.com/docs/10.x/eloquent-factories).
- Також, знайшов гарний опис на [habr](https://habr.com/ru/articles/645055/)

Наче немає нічого такого тяжкого, у разі чого, інфу можна буде глянут в коміті до цього уроку
